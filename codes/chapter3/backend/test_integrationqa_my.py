# 设定Python程序的运行环境以import backend的程序
import os,sys
...
datadir = "/home/ubuntu/llm-graph-builder/backend"
CHUNK_DIR = os.path.join(datadir, "chunks")
MERGED_DIR = os.path.join(datadir, "merged_files")
...
def test_graph_from_file_local(model_name):
   ...
   file_name = 'wukongzhuan-1-4.txt'
   shutil.copyfile('/home/ubuntu/dataset/test/input/wukongzhuan-1-4.txt',
...
def test_chatbot_qna(model_name, mode='vector'):
   ...
   QA_n_RAG = QA_RAG(graph, model_name, '唐僧遇到了什么？', '[]', 1, mode)
   print(QA_n_RAG)
...
if __name__ == "__main__":
   ...
   test_chatbot_qna(model_name)
   test_chatbot_qna(model_name, mode='graph+vector')
