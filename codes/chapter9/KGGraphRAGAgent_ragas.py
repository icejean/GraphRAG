...
def loadLLM2(vendor):
    if vendor=="Baidu":
        model = QianfanChatEndpoint(
            streaming=True, # Key setting
            endpoint="ernie-4.0-turbo-128k", # ernie-3.5-128k, ernie-4.0-turbo-128k
            temperature=0.1,
            timeout=120,
        )
    elif vendor=="Xunfei":
        ...
        model = ChatSparkLLM(
            ...
            spark_api_key = keys.api_key,
            spark_api_secret = keys.api_secret,
            model='Spark4.0 Ultra',  # Spark Pro-128K, Spark4.0 Ultra
            streaming=True,
            timeout=120
        )
    elif vendor=="Tengxun":
...
# Ollama qwen2.5可以跑出来，但是不够稳定，faithfulness算不出来。
# vLLM MiniCPM3可以都跑出来。
...
q1 = "孙悟空跟女妖之间有什么故事？"
ebd1 = embeddings.embed_query(q1)

q2 = "What is the relationship between Sun Wukong and the female demon in the story?"
ebd2 = embeddings.embed_query(q2)

q3 = "孙悟空与女妖之间的关系是什么样的？"
ebd3 = embeddings.embed_query(q3)


# 计算点积
dot_product = np.dot(ebd1, ebd2)
# 计算向量的模
norm_A = np.linalg.norm(ebd1)
norm_B = np.linalg.norm(ebd2)
# 计算余弦相似度
cosine_similarity1 = dot_product / (norm_A * norm_B)

# 计算点积
dot_product = np.dot(ebd1, ebd3)
# 计算向量的模
norm_A = np.linalg.norm(ebd1)
norm_B = np.linalg.norm(ebd3)
# 计算余弦相似度
cosine_similarity2 = dot_product / (norm_A * norm_B)
